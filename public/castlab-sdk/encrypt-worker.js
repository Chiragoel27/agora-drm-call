...
async function initializeEncryptor(videoCodec, aesMode, key, iv) {
    ...
    encryptor = new crypto.Encryptor(videoCodec === 'AV1' ? crypto.Codec.AV1 : crypto.Codec.AVC, aesMode === 'CTR' ? crypto.Mode.CTR : crypto.Mode.CBC, key, 1024 * 1024)
    if (aesMode === 'CBC')
...
onmessage = (event) => {
    ...
    if (event.data.operation === 'init') {
        console.log(`${new Date().toISOString().slice(11, -1)} starting to load encryptor (wasm)`)
        initializeEncryptor(event.data.videoCodec, event.data.aesMode, event.data.key, event.data.iv).then(() => {
